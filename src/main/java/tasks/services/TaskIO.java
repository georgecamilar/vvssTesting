...
public class TaskIO {
    ...
    public static void write(TaskList tasks, OutputStream out) throws IOException {
        try (DataOutputStream dataOutputStream = new DataOutputStream(out)) {
            ...
            for (Task t : tasks) {
                ...
                dataOutputStream.writeInt((int) t.getRepeatInterval());
                if (t.isRepeated()) {
    ...
    private static String getFormattedTask(Task task){
        ...
        if (task.isRepeated()){
            ...
            result.append(getFormattedInterval((int) task.getRepeatInterval()));
            result.append("]");
        }
        else {
            result.append(" at ");
    ...
    public static String getFormattedInterval(long interval){
        ...
        long days = interval/ SECONDS_IN_DAY;
        long hours = (interval - SECONDS_IN_DAY *days) / SECONDS_IN_HOUR;
        long minutes = (interval - (SECONDS_IN_DAY *days + SECONDS_IN_HOUR *hours)) / SECONDS_IN_MIN;
        long seconds = (interval - (SECONDS_IN_DAY *days + SECONDS_IN_HOUR *hours + SECONDS_IN_MIN *minutes));

        long[] time = new long[]{days, hours, minutes, seconds};
